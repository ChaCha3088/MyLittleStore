...
//        }
    ...
    public void deleteStoreTable(Long storeTableId, Long storeId) {
        ...
        //예외 발생
        if (orderRepository.findAllNotDeletedAndPaidByStoreId(storeTable.getOrder().getId()).size() > 0 | orderItemRepository.findAllByOrderId(storeTable.getOrder().getId()).size() > 0) {
